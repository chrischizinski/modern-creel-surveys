...
Chapters 12 through 16 covered the estimation and reporting workflow for a single survey season. Most management decisions, however, are not made from a single season's data. Is angling effort increasing or declining? Did a new size-limit regulation reduce harvest pressure on a targeted species? Has catch rate recovered to the level observed before a year of high fishing mortality? These questions require comparing estimates across years — and comparing estimates with uncertainty requires more care than comparing two point estimates.
...
Repeating this for six years, varying `catch_per_trip$mean`, `mean_total_anglers`, and `harvest$mean_pct` to reflect the fishery trajectory, produces the full simulated dataset. The estimation pipeline for each year follows the same pattern established in Chapters 12–14:
...
This two-period contrast is the simplest appropriate test for regulatory evaluation. It does not account for autocorrelation across years (fish populations in adjacent years are not independent), and it treats all pre-regulation years as exchangeable and all post-regulation years as exchangeable — assumptions that are reasonable here but would break down in a fishery with strong interannual environmental variation.
...
pred_piece <- harlan_annual |>
  group_by(regulation) |>
  reframe(year = seq(min(year), max(year), by = 0.05)) |>
  bind_cols(as.data.frame(predict(
    cpue_piece,
    newdata = harlan_annual |>
      group_by(regulation) |>
      reframe(year = seq(min(year), max(year), by = 0.05)),
    interval = "confidence"
  )))
...
When twelve or more years of data are available, generalized least squares with an AR(1) correlation structure (via `MASS::rlm()` or `nlme::gls()`) provides a more honest standard error for the trend slope. At six years, the weighted linear model with honest interpretation of the precision is the practical limit of what the data can support.

## Assembling a multi-year report table with season_summary() {#sec-trend-table}

For regulatory reporting, the raw annual estimates tibble is often restructured into a wide table with one row per year and columns for each metric. `season_summary()` assembles pre-computed `creel_estimates` objects into this wide format automatically. For years where the full design objects are not available — as in the pre-saved data here — the same table can be constructed directly from the estimates data frame:
...
Multi-year analysis of creel survey data requires treating each year's estimate as a quantity with uncertainty, not as a fixed datum. The contrast between two specific years uses a standard-error-propagated difference; trend analysis across many years uses precision-weighted regression. `compare_designs()` supports the two-year comparison with a tidy interface and forest-plot visualization. For the full multi-year table, either `season_summary()` (when design objects are available for all years) or a direct summarization of the estimates data frame produces the standard regulatory reporting format.

At simulated Harlan Reservoir, the before-after contrast on Walleye CPUE showed a decline in the transition period followed by partial recovery — a pattern consistent with the biological expectation for a size-limit change that temporarily removes larger fish from the "catchable" pool before the newly protected cohort grows into legal size. Whether that recovery is statistically distinguishable from year-to-year variability depends on the precision of the annual estimates, and the confidence intervals throughout this chapter make that uncertainty explicit rather than hiding it behind the point estimates alone.
