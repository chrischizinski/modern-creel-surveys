...
@fig-ba-contrast plots the precision-weighted period means with 95% confidence intervals, making the contrast visual. The horizontal gap between the two points is the estimated effect of the regulation; whether the intervals overlap determines whether the difference is statistically distinguishable.

```{r}
#| label: fig-ba-contrast
#| fig-cap: "Before-after contrast for Walleye CPUE at Harlan Reservoir. Points show precision-weighted mean CPUE for the pre-regulation (2018–2020) and post-regulation (2021–2023) periods; error bars show 95% confidence intervals on the period-level weighted means. The gap between periods represents the estimated regulatory effect; overlapping intervals indicate the difference is not statistically clear-cut at the 5% level."
#| fig-alt: "Dot plot with two points and error bars: pre-regulation CPUE on left, post-regulation on right, error bars overlapping."
...
The sign and magnitude of each period-specific slope tells a cleaner story than the single linear slope. The pre-regulation slope is `r round(slope_before, 4)` fish/hr/yr (`r if(slope_before < 0) "declining" else "increasing"`) and the post-regulation slope is `r round(slope_after, 4)` fish/hr/yr (`r if(slope_after > 0) "recovering" else "continuing to decline"`). A reversal from negative to positive slope — even if imprecisely estimated — is qualitatively consistent with a regulatory benefit, independent of the interaction p-value. @fig-piecewise shows the predicted trajectories from the interaction model overlaid on the data.
...
@fig-residuals plots the residuals over time. Positive-followed-by-positive or negative-followed-by-negative runs — where residuals of the same sign cluster in adjacent years — are the visual signature of positive autocorrelation. With only six points any pattern is anecdotal, but the plot gives a quick sanity check.

```{r}
#| label: fig-residuals
#| fig-cap: "Residuals from the precision-weighted linear CPUE trend model, plotted by year. Positive residuals (CPUE above the trend line) and negative residuals (below) that cluster in adjacent years indicate positive autocorrelation, which makes p-values from the regression model overly optimistic. With only six years, any visible pattern should be interpreted cautiously."
#| fig-alt: "Needle plot of regression residuals by year, showing year-to-year variation around the weighted trend line."
