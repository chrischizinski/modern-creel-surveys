...
The fetch functions return plain data frames with canonical `tidycreel` column names. These are the same objects that Chapters 9–11 use directly from `.rds` files. The only difference is origin: CSV files through a connection vs. pre-packaged R data objects.
...
Chapters 9 and 10 walk through the complete construction of a `creel_design` object. Chapter 12 begins the estimation sequence. The connection layer described in this chapter is the upstream step that ensures all of those functions receive data in the shape they expect.
