...
The simplest API connection requires only a base URL, one or more creel survey UIDs, and a schema. The schema here supplies only the `survey_type` — the API backend uses hardcoded field name mappings for the NGPC response format, not the column-mapping arguments used by the CSV backend.
