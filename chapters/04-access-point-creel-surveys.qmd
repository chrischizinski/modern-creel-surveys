...
This chapter works through the access-point design from first principles: how the frame is defined, how counts translate to effort estimates, how completed-trip interviews produce catch rates through the ratio-of-means estimator, and where the design is vulnerable to bias. The example throughout is Harlan Reservoir, where boat anglers depart and return through a primary ramp and a secondary shore-fishing area.

## Constructing the access-point frame

The access-point frame is the set of spatial-temporal opportunities that define the survey's population of inference. As Pope et al. ([-@pope_chapter]) emphasize, a frame that fails to enumerate real fishing activity produces estimates that undercount the true total, regardless of how carefully the covered access points are sampled.
...
The completed-trip feature of access-point surveys breaks that correlation. At departure, every party — short-trip or long-trip — has an equal probability of being intercepted (assuming the technician is present and interviews all departing parties). The selection probability does not depend on trip duration. This property is what justifies the ratio-of-means estimator.
