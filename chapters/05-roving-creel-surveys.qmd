...
This creates a censoring problem. The analyst observes catch-so-far and effort-so-far, but not the completed trip's catch and effort. The ratio of catch-so-far to effort-so-far — the mid-trip catch rate — is not the same as the ratio of completed-trip catch to completed-trip effort. It will equal the completed-trip rate only if the rate at which fish are caught is constant throughout the trip (stationary catch rate). That constancy assumption is rarely defensible: anglers move, feeding activity changes with light and temperature, and successful anglers either quit early when they reach a bag limit or fish longer to maximize harvest.
...
# estimate_catch_rate with MOR for incomplete trips
# Requires a creel_design object — see Chapter 10 for full pipeline
est <- estimate_catch_rate(
  design    = harlan_design,
  estimator = "mean-of-ratios",
  truncate_at = 0.5,
  by        = "species"
)
```

The `truncate_at` argument applies the 0.5-hour threshold automatically. The `estimator = "mean-of-ratios"` argument selects the Hoenig-recommended MOR formula rather than the ROM default. Chapter 10 covers the full `creel_design()` pipeline required to pass a design object to `estimate_catch_rate()`.

## Diagnosing incomplete trips with `validate_incomplete_trips`
...
# validate_incomplete_trips checks the roving interview structure
# Requires creel_design object — illustrative call
diagnostics <- validate_incomplete_trips(
  design      = harlan_design,
  catch       = harlan_catch,
  effort      = harlan_counts,
  truncate_at = 0.5
)
```
...
- Trips with zero elapsed time (usually data entry errors)
- Trips below the truncation threshold
- Trips where reported catch exceeds what is biologically plausible given elapsed duration
- Imbalanced incomplete-trip fractions across strata (if one stratum has disproportionately many short contacts, the truncation may introduce stratum-level bias)
...
# Requires creel_design object; illustrative syntax
summarize_trips(harlan_design)
```

The output shows counts of complete and incomplete interviews per stratum, mean elapsed effort hours by status, and the fraction below the truncation threshold — enough to diagnose whether the roving component is adequately sampled before committing to a full seasonal estimate.
