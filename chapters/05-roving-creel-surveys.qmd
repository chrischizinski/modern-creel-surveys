...
The output reports total interview counts, the complete/incomplete split, and summary statistics for elapsed duration by trip status. The 4.74-hour mean for complete trips and 1.71-hour mean for incomplete trips match the manual calculation from the length-of-stay comparison earlier in this chapter — confirming that the design object carries the same data used in the worked examples and that the trip-status routing is correct before estimation proceeds.
