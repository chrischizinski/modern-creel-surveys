...
Effort is derived from counts, not from interviews. An instantaneous count records how many anglers are present at a moment in time; that observation, multiplied by the length of the sampling period, yields an estimate of angler-hours. Interviews provide catch and fishing time per party, which enter the catch-rate calculation but not the effort estimate directly. This separation of sources is structural and intentional, and is explained in @sec-counts-not-trips.
...
The pattern is always the same: filter to the domain, compute the rate at the interview level, compute the effort estimate for the matching domain, multiply. Writing the specification before writing any code makes it visible whether each filtering step matches the stated domain.
