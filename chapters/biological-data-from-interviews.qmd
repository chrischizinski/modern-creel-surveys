...
The subsample rate also has a direct effect on precision. More measured fish within an interview reduces within-interview variability in the length column; more interviews with any length records reduces between-interview variability. For a fixed field budget, measuring a few fish across more interviews is more efficient than measuring many fish in fewer interviews, because between-interview variance dominates total variance in stratified survey estimators — a pattern visible in the variance decomposition for effort in Chapter 15.
...
Boat anglers show marginally higher compliance than bank anglers, consistent with their ability to target deeper water where larger fish concentrate. The difference is not statistically tested here — a formal comparison would require a design-weighted contrast — but the pattern is biologically plausible and worth noting in a regulatory report alongside the overall compliance figure.
...
For Walleye, a commonly used length-weight relationship takes the form $W = a \cdot L^b$ where $W$ is weight in grams and $L$ is total length in mm. Example parameters ($a = 0.0000106$, $b = 3.093$) are illustrative; programs should fit species- and water-body-specific coefficients from their own biological sampling data. Apply the relationship at each bin midpoint to approximate a distribution-weighted mean weight, then multiply by the estimated harvest total.
