...
Creel survey errors fall into two broad categories: **bias** and **variance**. Variance is the sampling variability that shrinks with more observations; it is managed through design choices like stratification and allocation. Bias is systematic error that does not shrink with more observations. Adding more interviews in the wrong place does not fix a coverage gap; it amplifies it.

Within bias, the literature identifies several recurring types [@pollock1994]:

**Coverage bias** arises when the sampling frame does not include the full target population. Common examples include private-launch access points omitted from the frame, nocturnal anglers who fish outside survey shift hours, and water bodies or zones excluded for logistical reasons.

**Nonresponse bias** arises when the anglers who are approached but decline to participate differ systematically from those who accept. Refusal rates are highest when conditions are poor, when anglers are packing up in a hurry, and when crews are unfamiliar or encounter language barriers.

**Avidity bias** arises because anglers who fish more often, or for longer trips, are more likely to be intercepted by an on-site survey than infrequent or short-trip anglers. The sample overrepresents the avid core of the fishery unless the estimator accounts for differential interception rates.

**Temporal bias** arises when the survey does not cover the full temporal extent of the fishery — missing weekday patterns, nighttime fishing, or entire months due to budget limits or weather cancellations.

**Spatial bias** arises when some fishing locations contribute disproportionately to the sample because the access point list is incomplete or because certain areas are sampled more intensively for non-statistical reasons.

**Operational errors** are not bias in the statistical sense, but they have the same practical effect. Missed shifts, recording errors, selective interviewing by inexperienced clerks, and species misidentification all alter the effective sample in ways that cannot be corrected by a better estimator.
...
These exclusions are not recorded in the data. They appear only in the survey design documentation. An analyst working from the data file alone would not know they existed.
...
The most plausible direction of that bias in a creel context is upward pressure on catch rates. Anglers who had an unsuccessful trip may be more likely to wave off the interviewer and leave quickly. Successful anglers, with fish to show and a story to tell, are more likely to linger. If this pattern holds, the interview sample overrepresents catching parties and the estimate overstates CPUE.
...
summarize_by_angler_type(harlan_design_ch08)
```
...
The Harlan 2022 field record does not include a log of cancelled or shortened shifts. Without that record, it is impossible to assess whether fair-weather bias affected the estimates. For future survey seasons, logging each planned shift and marking it as completed, shortened, or cancelled — and recording the reason — takes five minutes per shift and makes the temporal coverage audit tractable.
...
No creel survey answers all of these questions satisfactorily. The goal is not a perfect survey but an honest one — where the analyst can say clearly what the data support, what the design was able to claim, and what should be treated as a limitation rather than a result.

## Takeaway {#sec-bias-takeaway}

The biggest creel failures often happen before estimation starts.
