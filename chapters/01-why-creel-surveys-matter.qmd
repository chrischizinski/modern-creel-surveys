...
Recreational fishing is one of the most widely practiced outdoor activities in North America, yet the fisheries that support it are among the least precisely monitored. Unlike commercial fisheries, where landings are recorded at processing facilities and catch limits are enforced at the dock, recreational harvest is distributed across thousands of access points, compressed into short seasonal windows, and produced by anglers who have no legal obligation to report what they caught. The result is a system where meaningful amounts of fishing pressure and harvest can accumulate unobserved, and where the first indication that a population is in trouble may arrive years after the damage has been done [@pope_chapter].

Creel surveys exist to close that gap. By sending trained clerks into the field to count anglers and conduct structured interviews, fisheries agencies collect the raw material needed to estimate total effort, total catch, and angler behavior across a defined fishery and time period. Done well, a creel survey converts a partially observed, spatially distributed recreational fishery into a set of statistically defensible estimates that can be compared across years, presented to regulatory bodies, and used to justify changes in harvest regulations or stocking programs.

The economic stakes are substantial. The U.S. Fish and Wildlife Service estimates that recreational anglers spend tens of billions of dollars annually on equipment, travel, and license fees. In many states, sport fishing license revenue is the primary funding mechanism for fisheries conservation programs. The fish populations that support this activity are a public trust resource, and fisheries agencies are obligated — legally in some jurisdictions — to manage that resource using the best available information about harvest and effort. Creel surveys are the primary mechanism for generating that information.

The methodology is not new. Roving creel surveys were described in the North American literature by the 1950s, and the statistical foundations for estimation from instantaneous counts were formalized by @pollock1994 and colleagues at a symposium that produced a synthesis still widely cited today. What has changed is the computational environment. Modern creel datasets involve thousands of records, multiple strata, incomplete trips, species-level catch matrices, and the expectation that estimates will be accompanied by defensible variance estimates. Doing that work in spreadsheets — as many agencies still do — is error-prone, difficult to audit, and nearly impossible to replicate. This book is about how to do it reproducibly in R with the `tidycreel` package.
...
This book is organized around the estimation workflow, not around individual concepts. Each chapter takes the output of the previous chapter as its input and produces output that feeds into the next. The structure mirrors what a practitioner does when analyzing a real creel dataset from scratch.

Chapters 2 and 3 lay the statistical foundation: Chapter 2 defines estimands formally, introduces notation, and describes the reporting domains that bound creel inference. Chapter 3 develops the design-based estimator, derives the variance formula, and works through the Harlan dataset from raw counts to a stratified effort estimate with a confidence interval.

Chapters 4 through 8 address the estimation pipeline: total catch, harvest, release, catch rate, and the handling of incomplete trips. These chapters introduce the core `tidycreel` estimation functions and demonstrate their application to the Harlan dataset. Each chapter includes a section on diagnostics — how to check whether the estimator's assumptions are met and what to do when they are not.

Chapters 9 through 12 cover biological data: length frequencies, weight estimation, age composition, and their integration with catch estimates to produce biological reference points. These chapters draw on the `estimate_length_distribution()` and related functions.

Chapters 13 through 16 address design: power analysis for proposed surveys, stratification decisions, spatial designs for large reservoirs, and temporal allocation of sampling effort. These chapters use `creel_power()` and `generate_schedule()` to demonstrate prospective design evaluation before field work begins.

Chapter 17 applies all methods to a real published dataset, closing the loop from synthetic teaching data to documented agency practice.

## Recreational fisheries as partially observed systems {#sec-partial-obs}

Consider what a fisheries manager actually knows about recreational harvest on a 5,000-hectare reservoir on a given day. An unknown number of anglers are on the water, arriving and departing at irregular intervals. They fish from boats, from shore, from kayaks. They target different species with different gear. Some keep fish; most do not. Some will tell the truth about what they caught; a few will not. By evening, every angler has gone home, and the only trace of the day's activity is whatever was recorded by whoever happened to be in the field.
...
The statistical framework for access point estimation is well developed. Under certain conditions, the probability that an angler passes through the access point during the clerk's presence can be estimated, and the inverse of that probability serves as the expansion factor for observed catch. The `tidycreel` function `estimate_effort()` implements this framework with support for stratification by day type, period, and section.
...
The connection between an instantaneous count and an effort estimate requires two additional pieces of information: the total number of possible fishing hours in the day (the "day length," from first legal fishing hour to last), and the mean time anglers spend fishing per trip. Under certain assumptions, the ratio of anglers counted to the day length yields an estimate of total effort. These assumptions — chiefly that the count is truly instantaneous and that the departure rate from the fishery is approximately constant across the day — are discussed formally in Chapter 3.
...
This temporal proximity to the fishing activity is the defining advantage of creel surveys over mail or telephone recall methods: anglers interviewed as they leave the water have far better recall of the trip than anglers contacted weeks later [@pope_chapter]. The error in recalled catch counts grows rapidly with time between the catch event and the interview; same-day intercept interviews are far more accurate than retrospective reports.
...
This miniature example captures the full logical chain: count $\rightarrow$ effort $\rightarrow$ total catch $\rightarrow$ harvest/release split. Every function in `tidycreel` implements some portion of this chain on real stratified datasets. The formalism of Chapter 3 extends this logic to cover stratification, variance estimation, and incomplete trip correction.
...
Chapter 6 develops the full power analysis framework using `creel_power()`, which accepts a hypothesized CV target and returns the number of sampled days needed to achieve it under a range of variance assumptions. Running this calculation before the first field day — not after — is the single most effective way to avoid underpowered surveys.
...
A reproducible creel workflow reads raw data from source files, applies documented transformations in sequence, and writes estimates to output files or tables. The key constraint is that every step must be code — no manual adjustments in Excel between the raw data and the final estimate.

A minimal structure that satisfies this constraint:
...
This is the structure used throughout this book. Each chapter introduces functions that operate on tidy data frames, produce tidy outputs, and chain together without undocumented intermediate steps.
...
#| fig-cap: "Distribution of total boat-angler counts per sampled day at Harlan Reservoir, by day type. Each observation is the sum of boat anglers recorded across all count periods on a given day."
...
#| fig-cap: "Distribution of completed trip durations (hours fished) at Harlan Reservoir, by angler type. Only complete trips are shown; incomplete trips are excluded from this display."
...
#| fig-cap: "Total harvested fish by species at Harlan Reservoir across the 2022 survey season. Species are ordered by total harvest."
